# Mastermind
This is a program to guess the code. The algorithm implemented in this file can reach about 10.85 times of guessing in average. You can find two python file. The ```Game.ipynb``` file is a breakdown explanation on playing one time game. The ```mastermind.py``` file is to obtain the average number of guessing times after playing 10000 times. Recently, our algorithm is able to obtain average guessing time 10.38 for 1500 times of play. The result still need to be improved. 
## 1. About the game<br />
